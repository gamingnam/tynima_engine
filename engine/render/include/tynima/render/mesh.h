...
// Tangents join when normal mapping does (Phase 1, task 5).
struct Vertex {
    math::Vec3 position;
    math::Vec3 normal;
    math::Vec2 uv;
};
static_assert(sizeof(Vertex) == 32, "Vertex layout is assumed by vertex_layout() and the shaders");

// The layout of Vertex for pipeline creation: attribute 0 position, 1 normal, 2 uv.
