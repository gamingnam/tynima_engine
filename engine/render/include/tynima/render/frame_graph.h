...
// What a pass's execute callback gets: the physical texture behind a handle.
class PassResources {
public:
...
// Runs inside the pass; captures by reference or plain value only.
using PassExecute = core::InlineFunction<void(rhi::RenderPass&, const PassResources&), 128>;

// Given to a pass's setup callback to declare what the pass touches. Color
// attachments take the slots in the order they are declared.
class PassBuilder {
public:
...
public:
    ...
    void add_pass(const char* name, Setup&& setup, Execute&& execute) noexcept {
        const std::uint32_t index = open_pass(name);
        if (index == kNone) {
            return;
        }
...
private:
    ...
    std::uint32_t open_pass(const char* name) noexcept;
